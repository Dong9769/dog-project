...
pd_human = pd.Series(human_files_short, index=np.arange(len_human))
face_result = pd_human.apply(face_detector)
detect_face_num = pd.Series(face_result).sum()
print('human face detect percent: {:.2%}'.format(detect_face_num/100))

# dog detect
len_dog = len(dog_files_short)
pd_dog = pd.Series(dog_files_short, index=np.arange(len_dog))
dog_result = pd_dog.apply(face_detector)
detect_dog_num = pd.Series(dog_result).sum()
print('dog detect percent: {:.2%}'.format(detect_dog_num/100))
...
# 
# __回答:__
# （1）不合理，实际使用人可能是静止的也可能是运动的，有的动的慢有的动的快，不可能让中采集到的图像每个用户都按照要求采集图像。
# （2）在输入算法前对图像进行预处理，如图像增强、图像复原、超分辨率重构算法，目前都是比较成熟的图形去模糊算法，在进行检测前先对图像进行去模糊，去除噪声、增强有用信号，再进行检测，会提高检测的质量和有效性。
# 
...
# human face detect
len_human2 = len(human_files_short)
pd_human2 = pd.Series(human_files_short, index=np.arange(len_human2))
face_result2 = pd_human2.apply(dog_detector)
detect_face_num2 = pd.Series(face_result2).sum()
print('human face detect percent: {:.2%}'.format(detect_face_num2/100))

# dog detect
len_dog2 = len(dog_files_short)
pd_dog2 = pd.Series(dog_files_short, index=np.arange(len_dog2))
dog_result2 = pd_dog2.apply(dog_detector)
detect_dog_num2 = pd.Series(dog_result2).sum()
print('dog detect percent: {:.2%}'.format(detect_dog_num2/100))
...
# 
# __回答:__ 
# 我理解的卷积网络主要有三个部分，第一部分Conv2D与MaxPooling2D组成的卷积网络，用于加深数据深度，提取图像特征。第二部分完全连接层，主要用于将多维输入一维化，输入神经网络。第三部分神经网络用于根据卷积层提取的特征分类数据。
# 基于以上认识搭建的网络，首先卷积层部分，使用Conv2D和MaxPooling2D组合，输入的维度为（224,224,3)，输出后变为(3,3,64)。完全连接层使用Flatten函数将数组展平为576维的一维数组。然后输入到神经网络分类。最后输出层softmax激活函数，目标类133。
...
bbottleneck_features = np.load('/data/bottleneck_features/DogResnet50Data.npz')
train_Resnet50 = bbottleneck_features['train']
valid_Resnet50 = bbottleneck_features['valid']
test_Resnet50 = bbottleneck_features['test']
...
# 
# __回答:__ 
# 我选择使用ResNet50模型，使用该模型的原因是网络层级高，并且使用跳层连接的方法消除了梯度消失问题，应该可以很好地处理数据。
# 
# 我搭建的神经网络两个，一个是全局平局池化层，另一个是输出层。在使用Resnet50卷积模型提取的特征后没有使用全连接层，然后在增加多层神经网络分类数据，这样的方式参数太大。所以使用了全局平均池化层，即实现了全连接层的作用，还有效保留了数据的一些内在特征。输出层使用softmax激活函数，目标类133个。

# In[25]:


### TODO: 定义你的框架
Self_Resnet50_model = Sequential()
# train_Resnet50.shape[:]
# train_VGG16.shape[1:]

Self_Resnet50_model.add(GlobalAveragePooling2D(input_shape=train_Resnet50.shape[1:]))
Self_Resnet50_model.add(Dense(133, activation='softmax'))

Self_Resnet50_model.summary()
...
### TODO: 编译模型
Self_Resnet50_model.compile(loss='categorical_crossentropy', optimizer='rmsprop', metrics=['accuracy'])
...
### TODO: 训练模型
bcheckpointer = ModelCheckpoint(filepath='saved_models/weights.best.Resnet50.hdf5', verbose=1, save_best_only=True)

Self_Resnet50_model.fit(train_Resnet50, train_targets, validation_data=(valid_Resnet50, valid_targets), epochs=20, batch_size=20, callbacks=[bcheckpointer], verbose=1)
...
Self_Resnet50_model.load_weights('saved_models/weights.best.Resnet50.hdf5');
...
Resnet50_predictions = [np.argmax(Self_Resnet50_model.predict(np.expand_dims(feature, axis=0))) for feature in test_Resnet50]
...
def Resnet50_predict_breed(img_path):
    ...
    #获取预测向量
    Resnet50_predicted_vector = Self_Resnet50_model.predict(Resnet50_bottleneck_feature)
...
# 使用模型处理单个图像
def distinguish_categories(img_path):
    title = ''
    predict_dog_categories = ''
    
    if dog_detector(img_path):
        title = 'This is a dog'
        predict_dog_categories = Resnet50_predict_breed(img_path)
    elif face_detector(img_path):
        title = 'Hello, human'
        predict_dog_categories = 'You look like a ...\n' + Resnet50_predict_breed(img_path)
    else:
        title = 'error'
        predict_dog_categories = 'error'
    return (img_path,title, predict_dog_categories)

#设置plt中子图参数
def set_subplot(plt, rows, cols, num, data):
    plt.subplot(rows, cols, num)
    plt.title(data[1])
    plt.text(50, 280, data[2], ha='left', wrap=True)
    img1 = image.load_img(data[0], target_size=(224, 224))
    plt.imshow(img1)
    

#输入处理结果数组，显示结果
def show_results (plt_datas):
    objects_len = len(plt_datas)
    
    #plt子图列数
    cols = 3
    #plt子图行数
    rows = math.ceil(objects_len/cols)
    
    #设置窗口大小
    plt.figure(figsize=(20,10)) 
    # 图片名称
    plt.suptitle('Process result') 
    index = 0
    for obj in plt_datas:
        index = index + 1
        set_subplot(plt, rows, cols, index, obj)
...
# 输入图片途径数组，处理并显示结果 
def distinguish_and_show(img_paths):
    process_result = [distinguish_categories(img_path) for img_path in img_paths]
    show_results(process_result)
...
# 
# 答：正确分类了输入图片，比预期好。
# 改进想法：1.目前我对数据的处理未使用图像增强算法，可以使用图像增强算法，提高模型对各个角度、位置的对象识别以提高精度。2.使用更多层的神经网络，有益于使用特征更精细的分类，但是要注意到过拟合的点。3.调整训练步长参数，并观察精度，寻找不过拟合又合适的步长值。
...
test_files = np.array(glob("test_img/*"))

distinguish_and_show(test_files)
